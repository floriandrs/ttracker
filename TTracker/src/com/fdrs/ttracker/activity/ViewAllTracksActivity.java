...
import com.fdrs.ttracker.database.contentprovider.TrackContentProvider;
import com.fdrs.ttracker.database.cursoradapter.TrackCursorAdapter;
...
public class ViewAllTracksActivity extends ListActivity implements LoaderManager.LoaderCallbacks<Cursor> {
	...
	protected void viewTrackInMap(long id) {
		...
		Cursor c = getContentResolver().query(DbUtil.buildUri(id), TrackTable.getProjection(), null, null, null);
		c.moveToFirst();
		...
		if (Util.latLonIsEmpty(lat, lon)) {
			...
			String url = "http://maps.google.com/maps?t=h&q=loc:"+lat+","+lon+"&z=10";
